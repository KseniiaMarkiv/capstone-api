...
RSpec.describe "ThingImages", type: :request do
  ...
  describe "manage thing/image relationships" do
    ...
    context "valid thing and image" do
      let(:thing) { create_resource(api_things_path, :thing, :created) }
      let(:image) { create_resource(api_images_path, :image, :created) }
      ...
      it "can associate image with thing" do
        ...
        post api_thing_thing_images_path(thing["id"]), thing_image_props
        expect(response).to have_http_status(:no_content)

        #get ThingImages for Thing and verify associated with Image
        get api_thing_thing_images_path(thing["id"])
      ...
      it "must have image" do
        post api_thing_thing_images_path(thing["id"]), 
  ...
  shared_examples "can get links" do
    it "can get links for Thing" do
      get api_thing_thing_images_path(linked_thing_id)
      #pp parsed_body
      expect(response).to have_http_status(:ok)
    ...
    it "can get links for Image" do
      get api_image_thing_images_path(linked_image_id)
      #pp parsed_body
      expect(response).to have_http_status(:ok)
  ...
  shared_examples "get linkables" do |count, user_roles=[]|
    it "return linkable things" do
      get api_image_linkable_things_path(linked_image_ids[0])
      #pp parsed_body
      expect(response).to have_http_status(:ok)
  ...
  shared_examples "can create link" do
    it "link from Thing to Image" do
      post api_thing_thing_images_path(linked_thing_id), thing_image_props
      expect(response).to have_http_status(:no_content)
      get api_thing_thing_images_path(linked_thing_id)
      expect(parsed_body.size).to eq(linked_image_ids.count+1)
    end
    it "link from Image to Thing" do
      post api_image_thing_images_path(thing_image_props[:image_id]), 
    ...
    it "bad request when link to unknown Image" do
      post api_thing_thing_images_path(linked_thing_id), 
  ...
  shared_examples "can update link" do
    it do
      put api_thing_thing_image_path(thing_image["thing_id"], thing_image["id"]), 
  ...
  shared_examples "can delete link" do
    it do
      delete api_thing_thing_image_path(thing_image["thing_id"], thing_image["id"])
      expect(response).to have_http_status(:no_content)
    end
  end
  ...
  describe "ThingImage Authn policies" do
    ...
    let(:thing_resources) { 3.times.map { create_resource(api_things_path, :thing, :created) } }
    let(:image_resources) { 4.times.map { create_resource(api_images_path, :image, :created) } }
    ...
    before(:each) do
      ...
      linked_image_ids.each do |image_id| #link thing and images, leave orphans
        post api_thing_thing_images_path(linked_thing_id), {:image_id=>image_id}
        expect(response).to have_http_status(:no_content)
      end
    end
