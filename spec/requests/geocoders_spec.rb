...
RSpec.describe "Geocoders", type: :request do
  ...
  describe "geocoding" do
    ...
    context "API geo" do
      ...
      it "locates location by address" do
        ...
        expect(json).to include("formatted_address"=>geo.formatted_address)
        expect(json).to include("position"=>geo.position.to_hash.stringify_keys)
        expect(json).to include("address"=>geo.address.to_hash.stringify_keys)
      ...
      it "locates location by position" do
        ...
        expect(json).to include("formatted_address"=>rgeo.formatted_address)
        expect(json).to include("position"=>rgeo.position.to_hash.stringify_keys)
        expect(json).to include("address"=>rgeo.address.to_hash.stringify_keys)
        expect(response.header).to include("Cache-Control")
        expect(response.header["Cache-Control"].match(/max-age=(\d+),/)[1]).to eq("86400")
      end
    end
  ...
  describe "Image position" do
    it "can return Image position" do
      ...
      expect(json["position"]).to_not be_nil
      expect(json["position"].symbolize_keys).to eq(image.position.to_hash)
    end
    ...
    it "can set Image position" do
      ...
      json=parsed_body
      expect(json["position"]).to_not be_nil
      expect(json["position"].symbolize_keys).to eq(image_props[:position])
    end
  end
  ...
  describe "search" do
    ...
    describe "search origin" do
      it "within range by position" do
        ...
        json=parsed_body
        expect(json.size).to eq(results.size)
        expect(json[0]).to include("thing_id", "thing_name")
        expect(json[0]).to include("image_id", "image_caption", "image_content_url")
        expect(json[0]).to include("position")
      end
    end
    ...
    describe "search subject" do
      ...
      it "finds things" do
        ...
        get subjects_path, {miles:@distance,subject:Thing.name}.merge(@origin.to_hash)
        #pp parsed_body
        expect(response).to have_http_status(:ok)
      ...
      it "finds images" do
        ...
        get subjects_path, {miles:100,distance:true}.merge(@origin.to_hash)
        #pp parsed_body
        expect(response).to have_http_status(:ok)
        json=parsed_body

        expect(json.size).to be >= images_without_things.size
        found=0
        json.each do |ti|
    ...
    shared_examples "ordered" do |direction|
      it "ordered" do
        results=ThingImage.within_range(@origin, @distance)
        get subjects_path, {miles:@distance,distance:true,order:direction}.merge(@origin.to_hash)
        #pp parsed_body
        expect(response).to have_http_status(:ok)
    ...
    describe "results finishing" do
      ...
      it "has distance from origin" do
        results=ThingImage.within_range(@origin, @distance)
        get subjects_path, {miles:@distance,distance:true}.merge(@origin.to_hash)
        expect(response).to have_http_status(:ok)
    ...
    describe "result caching" do
      before(:each) do
        get subjects_path, {order: :ASC}.merge(@origin.to_hash)
        expect(response).to have_http_status(:ok)
      ...
      context "updates eTag-modified for ThingImage" do
        ...
        it "updates last-modified for ThingImage delete" do
          ...
          delete thing_thing_image_path(ti.thing, ti)
